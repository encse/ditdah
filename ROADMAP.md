...
- [x] Persist whether Settings has ever been saved and automatically open it once at application startup while it remains unconfigured.
...
- [x] Add Linux and Windows builds for the pinned static Hamlib dependency, link them into the native release binaries, and statically link the MinGW runtime so the Windows executable does not require `libwinpthread-1.dll`.
...
- [x] Strip symbol tables and DWARF data from release binaries while retaining Go runtime stack traces.
- [x] Publish releases through one manually dispatched workflow which validates the version, builds every platform before tagging, then creates the tag and GitHub Release.
...
- [x] Bound long header status messages to the available space and append an ellipsis instead of allowing errors to cover the application menu.
