...
- [x] Bind background work and queued UI updates to the exact owner layer; pin both in the layer task group, discard updates after that layer leaves the requested stack, and wait for running UI callbacks during shutdown.
- [x] Keep confirmation dialogs UI-only: close and invoke their callback on confirmed OK; let Cancel and Escape only close, and let the owning page start work and present operation errors separately.
